...
vector<Mat> create_n_images(const string& original_image_path, int n) {
    ...
    // Create N different images, each with a specific subset of the colored pixels and the rest as transparent
    vector<Mat> images;
    for (size_t i = 0; i < divided_colored_pixels.size(); i++) {
        Mat colored_image = create_colored_image(image, divided_colored_pixels[i]);
        images.push_back(colored_image);
    }
